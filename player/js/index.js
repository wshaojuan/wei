...
var timer,//计时器
    ...
    img = ['./img/xul.jpg','./img/xux.jpg','./img/xue.jpg'],
    index = 0,
    newvol = 20,//开始播放音乐时，默认的音量大小是0.25
...
function init(){
    ...
    oBtn2.onmouseup = function (e) {
        ...
        if(oAudio.paused) {
            musicPlay();
            oAudio.volume = newvol/120;//开始播放音乐时，默认的音量大小是0.25
...
oRadioBox1.onmousedown = function() {
   document.body.onmousemove = function(e){
      newvol = 125 - (e.clientY - oVolBox.getBoundingClientRect().top);
      if(newvol<0){
          newvol = 0;
      }else if(newvol>120){
        newvol = 120;
